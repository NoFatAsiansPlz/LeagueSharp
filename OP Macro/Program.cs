...
namespace HaydariGeceler_cici_wipi_ENG
{
    internal class Program
    {
        ...
        private static string riot = " [RiotGames®] We added a new feature! Type fiddlix without pressing enter!";
            ...
            press3.ValueChanged += delegate(object sender, OnValueChangeEventArgs EventArgs)
            {
                if (haydarigeceler.Item("XDyaz").GetValue<KeyBind>().Active)
                    if (duramk == false)
                    {
                        ...
                        var message = string.Format("/all {0}{1} {2}", IsMe, new string(' ', 75 - (championName + summonerNameLength)), riot);
                        Game.Say(message);
                        duramk = true;
                        gameTime1 = Game.Time + 1;
